...
def compute_band_powers(eegdata, fs):
    ...
    # alpha
    ind_8hz, = np.where((f >= 6) & (f < 8))
    ind_alpha, = np.where((f >= 8) & (f < 12))

    mean_8hz = np.mean(PSD[ind_8hz, :], axis=0)
    mean_high_alpha = np.mean(PSD[ind_alpha, :], axis=0)
    
    # use this for ratio
    # feature = mean_alpha/mean_8hz
    # use this for alpha only
    # feature = mean_alpha

    return mean_8hz, mean_high_alpha
...
def update_buffer(data_buffer, new_data, notch=False, filter_state=None):
    ...
    # print("buffer shape ", new_buffer.shape)
...
def get_last_data(data_buffer, newest_samples):
    ...
    # print('data buffer shape ', data_buffer.shape)
    # print('newest sampe ', newest_samples)
    return new_buffer